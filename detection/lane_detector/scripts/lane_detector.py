...
# LANEWIDTH = 1  # highway lane width in US: 3.7 meters -> 1meter
# input_scale = 4
...
# y = [SKYLINE, SKYLINE, frame_height, frame_height]
# X = [0, BIRDVIEW_WIDTH, BIRDVIEW_WIDTH-BIRDVIEW_BOTTOM_DELTA, BIRDVIEW_BOTTOM_DELTA]
# Y = [0, 0, 0, 0]
...
def find_edges(image):
    ...
    min_threshold = (0, 0, 180)
    max_threshold = (179, 65, 255)

    binary = cv2.inRange(img_hsv, min_threshold, max_threshold)
...
if __name__ == '__main__':
    rospy.init_node('lane_detector_py')

    bridge = CvBridge()

    image_sub = rospy.Subscriber('/camera/rgb/image_raw', Image, image_callback, bridge)

    print("Start lane_detector!!")
    rospy.spin()
